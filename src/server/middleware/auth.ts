...
async function adminAuth(req: ExtendedRequest, res: ExtendedResponse, next: any) {
    try {
        const authHeader = req.headers['authorization']
        if (!authHeader) return next(new NotAuthorized("Token Required for authorization"))
