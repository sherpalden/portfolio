...
const uploadFileLocal = (fileObj: IReqFile) => {
  return (req: ExtendedRequest, res: ExtendedResponse, next: any) => {
    ...
    busboy.on("file", (fieldname, file, filename, encoding, mimetype) => {
      ...
      const key = fileObj.model
        ? `${fileObj.model}-${randomStr}-${fname}`
        : `default-${randomStr}-${fname}`;
      const filePath = path.resolve(`./public/uploads/${key}`);
    ...
    busboy.on("finish", async () => {
      ...
      if (hasErrorOccurred)
        return next(
          new BadRequest(
            `Only ${fileObj.fileNumberLimit} files can be uploaded`
          )
        );
      next();
    });
...
export { uploadFileLocal, deleteFile, deleteFiles };
